...
document.addEventListener('DOMContentLoaded', async() => {
    ...
    function sizeLayerControl() {
      let h=$("#map").height() - 50;
      $(".leaflet-control-layers").css("max-height", `${h}px`);
    }
    ...
    const apiHeaders={
        ...
        method: ( (mode=="prod" && window.navigator.onLine) ? "POST" : "GET" )
    };
    // ====================== init data here ===========================
    let apiUrl="";
    let response="";
    ...
    L.Control.PanelInfo = L.Control.extend({
      ...
      onAdd: function (map) {
          ...
          htmlContent+='<li class="active"><a data-toggle="tab" class="pt-4 pb-4 pl-2 pr-2 ml-0 mr-0 mb-0" href="#service_route_details_tab"><svg class="icon icon-route"><use xlink:href="symbol-defs.svg#icon-route"></use></svg> <small>Route</small></a></li>';
      
          htmlContent+='<li><a id="bus_eta_details_pill" data-toggle="tab" class="pt-4 pb-4 pl-2 pr-2 ml-0 mr-0 mb-0" href="#bus_eta_details_tab"><svg class="icon icon-bus-eta"><use xlink:href="symbol-defs.svg#icon-bus-eta"></use></svg> <small>ETAs</small></a></li>';
         

          htmlContent+='<div class="ml-0 mr-1 mt-3 mb-3 text-center float-right">';
          htmlContent+='<button id="toggleInfoPanel" type="button" class="btn btn-sm btn-outline-primary rounded-circle m-1 p-0 navBtn text-center unicode">◭</button><small class="ascii-chars text-primary">ᵀᵒᵍᵍˡᵉ</small>';
          htmlContent+='</div>';
          ...
          htmlContent+='<div class="ml-1 mr-1 mt-3 mb-3 text-center float-right">';
          ...
          htmlContent+='<thead>';
          htmlContent+='<tr>';
          htmlContent+='<th colspan="3" class="p-2">Please proceed to <svg class="icon icon-hand-pointer"><use xlink:href="symbol-defs.svg#icon-hand-pointer"></use></svg> select a bus service.</th>';
          htmlContent+='</tr>';
          htmlContent+='</thead>';
    ...
    $('#service_route_details_tab table thead').css({
      'display':''
    });
    $('#service_route_details_tab table tbody').css({
      'display':'none'
    });
